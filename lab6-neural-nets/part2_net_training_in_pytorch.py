...
def training(model, x, y):
    ...
    n_steps = 70000
    learning_rate = 0.03  # try different values
    minibatch_size = 32
    ...
    for _ in range(n_steps):
        ...
        indexes = torch.randint(0, x.shape[0], (minibatch_size,))
        x_batch = x[indexes]
        y_batch = y[indexes]
        ...
        # Ręczna aktualizacja parametrów
        with torch.no_grad():
            for p in model.parameters():
                p -= learning_rate * p.grad
...
def classify_spirals(student_id, do_data_inspection=True, do_model_inpection=True, do_model_training=True,
    ...
    model = TorchMultiLayerNetwork(
        ...
        # n_hiddens=[2],
        # n_hiddens=[16, 16],
        # n_hiddens=[16, 32, 64, 64, 64, 32, 16],
        n_hiddens=[16, 32, 32, 32, 32, 32, 32, 32, 16],
...
if __name__ == '__main__':
    student_id = 193257  # Twój numer indeksu, np. 102247
